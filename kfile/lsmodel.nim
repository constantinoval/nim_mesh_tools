...
proc readMeshSerial*(ls: var LSModel, meshPath: string): bool {.discardable.} =
    var
        mode = KeywordBlock.Undefined
        f = memfiles.open(meshPath)
        threelines: array[3, string]
        num_lines: uint = 0
    ...
    for l in memSlices(f, delim='\n'):
        ...
        case mode:
        of KeywordBlock.Node:
            let nd = fenode.fromStringFast($l)
            ls.nodes[nd.n] = nd
        of KeywordBlock.Solid:
            let el = feelement.fromStringFast($l, etype=Etype.Solid)
            ls.solids[el.n] = el
        of KeywordBlock.SolidOrtho:
            if num_lines<3:
                threelines[num_lines] = $l
                num_lines.inc
            else:
                let el = feelement.fromStringFastOrtho(threelines)
                ls.solidsortho[el.n] = el            
                num_lines = 0
        of KeywordBlock.Shell:
            let el = feelement.fromStringFast($l, etype=Etype.Shell)
            ls.shells[el.n] = el
        else:
            discard
    f.close()
...
proc calculateElementVolumesSerial*(model: var LSmodel) =
    ##[
...
proc saveSerial*(self: LSmodel, file_path: string) = 
    ##[
